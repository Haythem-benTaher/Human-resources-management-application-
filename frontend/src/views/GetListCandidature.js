...
const GetCandidature = () => {
  ...
  const [userID, setUserID] = React.useState(null); // State for userID
  
  React.useEffect(() => {
    // Retrieve the userId from session/localStorage
    const storedUser = JSON.parse(localStorage.getItem('user'));
    setUserID(storedUser?._id); // Get userId from localStorage

    document.documentElement.scrollTop = 0;
    document.scrollingElement.scrollTop = 0;

    if (storedUser?._id) {
      fetchData(storedUser._id); // Fetch data based on userId
    }
  }, [currentPage]);
  ...
  const fetchData = async (userId) => {
    ...
    try {
      ...
      const filteredData = data.filter(person => person.userId === userId);

      // Fetch user data for each candidature and add it to the candidature data
          ...
          <section className="section section-lg">
            <Container>
              <Row className="justify-content-center text-center mb-lg">
                <Col lg="8">
                  <h2 className="display-3">Check your applications</h2>
                </Col>
              </Row>
              {totalItems === 0 ? (
                  ...
                  <Col lg="8" className="text-center">
                    <Card className="h-100">
                      <div className="card-body text-center">
                        <p>No application found for you.</p>
                      </div>
                    </Card>
                  </Col>
                </Row>
